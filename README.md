...
**Spot preselect from hint (plan step 21):** a minimap `locationHint` exact-matches a spot **name** (case-insensitive, never fuzzy, never the area label). Preselect runs on an **appended** tick. A hint that exactly names one spot the account already owns is used **immediately**, with no stability window behind it — matching a closed vocabulary is a stronger signal than reads agreeing with each other, and minimap OCR of a zone label is erratic enough (`Sel Mahum Base` coming back as `Selfrlahum Base`, `SelM@hum Base`, `SOI Måhüm Base`…) that the 4-of-5 window can go a whole session without ever settling. A discarded OCR tick does not move the picker; a miss leaves it as it was. Save with an empty picker uses that same name. Never auto-saves.

The window still governs everything the shortcut does not cover: a name that is **not** already a spot has to settle (last 5 non-empty accepted hints, 4/5 the same — what the poll warm-up above counts towards) before Save will create it under World, because creating a spot from a single read would put an OCR misreading into the account permanently. The current reading must also still show that name, so a move in the last tick does not file under the old majority.

**Spot correction (`SpotAutoCorrect`):** because the hint outranks a pick that may be stale, a matching read may also **overwrite** a spot already in the picker, and says so — `Spot switched to "…"`. The player gets the last word: a pick made in response to that notice pins the session where they put it for the rest of the run, and nothing overwrites it again. The pin survives **Clear** on purpose — emptying the picker must not hand back protection already earned, or re-picking the same spot afterwards would silently fail to pin — and only a change of vocabulary drops it: a character switch or a reloaded spot list. Filling an *empty* picker is an ordinary preselect, not a correction — it shows no notice and does not arm the pin, so an ordinary session start never spends that one chance to overrule. The notice itself is transient: it retires as soon as a later read still names the spot in the picker, so the hint slot goes back to reporting the live location instead of announcing a switch from long ago. Being corrected is what entitles the player to pin, and that entitlement outlives both the notice and a trip through Clear. Once pinned, `SpotLocationWarning` takes over: it warns when the pinned spot has drifted from where the reads say the player is, and it reads the same evidence the correction did, so it still speaks when nothing has settled. Note a correction re-targets the **whole** log — the Play Report spans the entire session — so a walk to another known spot late in a long session attributes all of it to the new one; the picker and the notice are what make that visible before Save, which is always a manual click.
