...
**Auth (plan step 17):** paste the website JWT (`localStorage` key `l2_jwt_token`). It is stored DPAPI-encrypted (`%LOCALAPPDATA%\L2TrackerCompanion\auth.bin`, current-user scope) only after `GET /api/me` and then `GET /api/characters` succeed. A rejected token is deleted, not left on disk. Default API base is `https://l2tracker.cc` (editable in **Debug** mode).
