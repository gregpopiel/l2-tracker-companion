...
**Sign-in gate:** the token form is the first and only view at startup. The Session and Settings tabs are hidden until `GET /api/me` + `GET /api/characters` accept the token, and the app drops straight back to this screen on **Sign out** or if the stored token disappears mid-session. Reaching the gate stops tracking (the Stop button goes with the Session tab), and **Sign out** additionally wipes the local session store, so unsaved snapshots cannot be posted to whichever account signs in next — it asks for confirmation first whenever that store holds a savable delta. **Retry with stored token** appears whenever `auth.bin` still exists, and re-runs the same validation without a paste. The User / Debug radios sit here too (so a custom API URL can still be set on a fresh install, Debug only); the choice is the same one shown on Settings.

**Settings tab:** **Options** (User / Debug) and **Account** (signed-in status, **Sign out**) live here, not on the Session tab. Token entry and the API URL controls are on the sign-in screen, not here — on that screen **User** mode (the default) hides the API URL box and shows a non-default base as a read-only `API: …` line instead, and **Debug** turns it into an editable field. User mode also hides capture dumps, parse tools, and session inspect on the Session tab. The choice is stored in `%LOCALAPPDATA%\L2TrackerCompanion\options.txt` (`user` / `debug`).
